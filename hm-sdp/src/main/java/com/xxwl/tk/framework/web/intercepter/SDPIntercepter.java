...
public class SDPIntercepter implements HandlerInterceptor {
	...
	private void loadTerminalContext(HttpServletRequest request) {
		TerminalContext terminalContext = LocalSession.getCurrentTerminal(request.getSession());
		//同一个session只拦截一次
		...
		LocalSession.setAttribute(request.getSession(),CommonAttribute.TERMINAL_CONTEXT, terminalContext);
