...
/** 
* @ClassName: HmMemgradeController 
* @Description: 本类是由代码生成器自动生成HmMemgradeEntity逻辑(Controller)层
* @company 
* @author yixiang.deng
...
*/ 
@Controller
@RequestMapping("/tk/main/memgrade")
public class HmMemgradeController extends BaseController {
	//日志
	private static Logger logger = LoggerFactory.getLogger(HmMemgradeController.class);

	@Resource
	private HmMemgradeService hmMemgradeService;

	/** 
	* @Title: doAdd 
	* @Description: 新增操作
	* @param hmMemgradeEntity  Post提交
	*/ 
	@RequestMapping(value="/doadd",method = RequestMethod.POST)
	public @ResponseBody MessageDTO doAdd(HmMemgradeEntity hmMemgradeEntity,HttpServletRequest request,HttpServletResponse response) {
		// 1.服务器校验
		if(!doNullValidation(hmMemgradeEntity)){ 
			return this.responseData(false, null,MessageAttribute.COMMON_ERROR_VAL_EMPTY_OBJ);
		}
		//2.业务层调用
		Integer doAdd = hmMemgradeService.doAdd(hmMemgradeEntity);
		//3.返回JSON数据
	...
	/**
	 * @Title: 更新操作
	 * @Description: 更新HmMemgradeEntity 表单对象
	 * @param hmMemgradeEntity表单DTO对象
	 */
	@RequestMapping(value="/doupdate",method = RequestMethod.POST)
	public @ResponseBody MessageDTO doUpdate(HmMemgradeEntity hmMemgradeEntity,HttpServletRequest request,HttpServletResponse response) {	
		// 1.服务器校验
		if(!doNullValidation(hmMemgradeEntity)){ 
			return this.responseData(false, null,MessageAttribute.COMMON_ERROR_VAL_EMPTY_OBJ);
		}
		...
		// 3.校验成功，进行业务处理
		Integer doUpdate = hmMemgradeService.doUpdate(hmMemgradeEntity);
	...
	private MessageDTO deleteById(Integer id){
		...
		// 2.校验成功，进行业务处理
		Integer doDelete = hmMemgradeService.doDelete(id);
   ...
   /**
	...
	private MessageDTO removeById(Integer id){
		...
		// 2.校验成功，进行业务处理
		Integer doRemove = hmMemgradeService.doRemove(id);
	...
	private MessageDTO selectById(Integer id) {
		...
		// 2.校验成功，进行业务处理
		HmMemgradeEntity hmMemgradeEntity = hmMemgradeService.getById(id);
		return this.responseData(true, hmMemgradeEntity,MessageAttribute.COMMON_SELECT_VAL_SUC);
	}	
	...
	public @ResponseBody MessageDTO pageQuery(Criteria<HmMemgradeEntity>  hmMemgradeCriteria){
		// 1.服务器校验
		if(!doNullValidation(hmMemgradeCriteria)){ 
			return this.responseData(false, null,MessageAttribute.COMMON_ERROR_VAL_EMPTY_OBJ);
		}
		PageBean<HmMemgradeEntity> pageQuery = hmMemgradeService.pageQuery(hmMemgradeCriteria);
	...
	public @ResponseBody MessageDTO queryForList(Criteria<HmMemgradeEntity>  hmMemgradeCriteria) {
		// 1.服务器校验
		if(!doNullValidation(hmMemgradeCriteria)){ 
			return this.responseData(false, null,MessageAttribute.COMMON_ERROR_VAL_EMPTY_OBJ);
		}
		//查询条件
		List<HmMemgradeEntity> hmMemgradeEntityList = hmMemgradeService.queryForList(hmMemgradeCriteria);
		return this.responseData(true, hmMemgradeEntityList,MessageAttribute.COMMON_SELECT_VAL_SUC);				
	}		

    
	
	
	private boolean doUpdateValidation(HmMemgradeEntity hmMemgradeEntity) {
		boolean status = true;
		// 1.空对象校验
		if (hmMemgradeEntity == null) {
			status = false;
		}
		return status;
	}
	...
	* @return
	*/
	protected <T> boolean doAddValidation(HmMemgradeEntity hmMemgradeEntity ) {
		boolean status = true;
		// 1.空对象校验
		if (hmMemgradeEntity == null) {
			status = false;
